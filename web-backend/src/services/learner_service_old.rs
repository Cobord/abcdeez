...
impl LearnerService {
    ...
    pub async fn get_learner(&self, learner_id: Uuid) -> Result<Learner> {
        ...
        let user_id = row
            .user_id
            .map(|bytes| Uuid::from_bytes(bytes.try_into().unwrap_or_default()));
    ...
    pub async fn update_learner_model(
        &self,
        learner_id: Uuid,
        ...
        learner
            .core_model
            .update_memory_strength(&response.task_id, response.correct);

        if let Some(operation) = &response.operation_type {
            learner
                .core_model
                .update_operation_proficiency(operation, response.correct);
        }
        ...
        self.cache_learner_model(&learner_id, &learner.core_model)
            .await?;
    ...
    pub async fn export_learner_data(&self, learner_id: Uuid) -> Result<serde_json::Value> {
        ...
        let session_ids: Vec<Vec<u8>> = sessions.iter().map(|s| s.id.clone()).collect();
    ...
    async fn get_cached_learner(&self, learner_id: &Uuid) -> Result<Learner> {
        ...
        let model_data: String = crate::cache::cmd("GET")
            .arg(&model_cache_key)
            .query_async::<()>(&mut conn)
            .await
