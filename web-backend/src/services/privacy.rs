...
impl PrivacyBudget {
    pub fn new(epsilon_total: f64, delta_total: f64) -> Self {
        Self { epsilon_total, delta_total, epsilon_spent: 0.0, delta_spent: 0.0 }
    }

    pub fn remaining(&self) -> (f64, f64) {
        (self.epsilon_total - self.epsilon_spent, self.delta_total - self.delta_spent)
    }
...
impl DifferentialPrivacyEngine {
    pub fn new(epsilon: f64, delta: f64) -> Self {
        Self { budget: PrivacyBudget::new(epsilon.max(0.0), delta.max(0.0)) }
    }

    pub fn add_noise(&mut self, true_value: f64, mechanism: Mechanism, epsilon_cost: f64) -> f64 {
        match mechanism {
            Mechanism::Laplace { sensitivity } => {
                if !self.budget.spend(epsilon_cost, 0.0) { return f64::NAN; }
            ...
            Mechanism::Gaussian { sensitivity, delta } => {
                if !self.budget.spend(epsilon_cost, delta) { return f64::NAN; }
            ...
            Mechanism::RandomizedResponse { p } => {
                ...
                if flip { if rng.gen::<bool>() { 1.0 } else { 0.0 } } else { true_value }
            }
        }
    }
