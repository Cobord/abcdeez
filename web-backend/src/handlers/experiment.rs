...
pub async fn create(
    ...
    let query =
        "INSERT INTO experiments (id, name, description, config, status) VALUES (?, ?, ?, ?, ?)";
    let mut conn = state
        .db_pool
        .acquire()
        .await
...
pub async fn join(
    ...
    let mut conn = state
        ...
        .await
        .map_err(|e| AppError::DatabaseError(e))?;
    let rows = sqlx::query(check_query)
        .bind(id.to_string())
        .fetch_all(&mut *conn)
        .await
        .map_err(|e| AppError::DatabaseError(e))?;
...
pub async fn results(
    ...
    let result_rows = sqlx::query(results_query)
        .fetch_all(&mut *conn)
        .await
        .map_err(|e| AppError::DatabaseError(e))?;
...
pub async fn export(
    ...
    let export_rows = sqlx::query(export_query)
        .fetch_all(&mut *conn)
        .await
        .map_err(|e| AppError::DatabaseError(e))?;

    let data: Vec<serde_json::Value> = export_rows
        .into_iter()
        .map(|row| {
            serde_json::json!({
