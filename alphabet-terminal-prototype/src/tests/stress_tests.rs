...
fn test_numerical_stability_extreme_values() {
    ...
    assert!(!entropy.is_nan(), "Entropy should not be NaN for tiny variance");
    assert!(entropy.is_finite() || entropy == 0.0,
            "Entropy should be finite or zero: {}", entropy);
    ...
    assert!(kl.is_finite() || kl == f64::INFINITY,
            "KL should be finite or infinity: {}", kl);
    
    // Test with zero variance
    let zero_var = PosteriorDistribution::new(5.0, 0.0);
    assert_eq!(zero_var.entropy(), 0.0, "Zero variance should give zero entropy");
    ...
    let test_points = vec![
        -1000.0, -1.0, 0.0, 999.0, 1000.0, 1001.0, 10000.0
    ];
    
    for x in test_points {
        let pdf = model.pdf(x);
        assert!(pdf == 0.0 || pdf.is_finite(),
                "PDF at {} should be 0 or finite: {}", x, pdf);
...
fn test_overflow_underflow_prevention() {
    ...
    assert!(exp_result == f64::INFINITY,
    ...
    for x in extreme_x {
        ...
        assert!(pdf.is_finite() || pdf == 0.0,
                "PDF should be finite or zero at x={}: {}", x, pdf);
    }
}
...
fn test_division_by_zero_prevention() {
    ...
    let analysis = StrategyAnalysis::analyze(&y, &x.iter().map(|&v| v as usize).collect::<Vec<_>>());
    assert!(analysis.rt_distance_correlation.is_finite() || 
            analysis.rt_distance_correlation == 0.0,
    ...
    assert_eq!(empty_analysis.rt_distance_correlation, 0.0,
...
fn test_concurrent_model_updates() {
    ...
    assert!(entropy >= 0.0 && entropy.is_finite(),
            "Model should remain valid after concurrent updates: entropy={}", entropy);
    
    // Check that updates were applied
    let initial_model = BayesianLearnerModel::new(&topo);
    let initial_entropy = initial_model.total_entropy();
    assert!(entropy < initial_entropy,
            "Entropy should decrease after updates: {} < {}", entropy, initial_entropy);
}

#[test]
fn test_large_scale_performance() {
    // Test with large topology
    let large_nodes: Vec<String> = (0..100)
    ...
    assert!(init_time.as_millis() < 1000,
            "Initialization should be fast even for 100 nodes: {:?}", init_time);
    
    // Test EIG calculation performance
    let task = Task {
        task_type: TaskType::Successor { item: "Node_50".to_string() },
        prompt: "".to_string(),
    ...
    assert!(eig_time.as_millis() < 500,
            "EIG calculation should be reasonably fast: {:?}", eig_time);
}
...
fn test_nan_propagation_prevention() {
    ...
    assert!(dist.mean.is_finite() || dist.mean == f64::INFINITY,
    ...
    assert!(kl == f64::INFINITY || kl.is_finite(),
...
fn test_outlier_detection_edge_cases() {
    ...
    assert_eq!(outliers.len(), 0, "Should detect no outliers in normal data");
    ...
    assert!(outliers.contains(&4), "Should detect extreme outlier at index 4");
...
fn test_boundary_condition_validation() {
    ...
    if !model.chunk_boundaries.is_empty() {
        ...
        assert!(first_boundary.position < 26,
                "Chunk boundary position {} should be within alphabet", 
                first_boundary.position);
        assert!(first_boundary.strength.mean >= 0.0 && first_boundary.strength.mean <= 1.0,
