...
fn test_monte_carlo_convergence_rate() {
    ...
    assert!(last_three_variance < 0.01, 
            "Should converge with large samples: variance={}", last_three_variance);
}
...
fn test_correlation_significance_testing() {
    ...
    assert!(p_value < 0.001, "Should be highly significant: p={}", p_value);
...
fn test_bootstrap_confidence_intervals() {
    ...
    assert!(ci_lower <= true_mean && true_mean <= ci_upper,
            "True mean {} should be in CI [{}, {}]", true_mean, ci_lower, ci_upper);
    
    // Confidence interval should be reasonable width
    let width = ci_upper - ci_lower;
    assert!(width > 0.5 && width < 3.0,
            "CI width should be reasonable: {}", width);
}
...
fn test_hypothesis_testing_framework() {
    ...
    assert!(p_value > 0.05, 
            "Should not reject null hypothesis: t={}, p={}", t_stat, p_value);
    ...
    assert!(p_value2 < 0.001,
            "Should reject null hypothesis: t={}, p={}", t_stat2, p_value2);
}
...
fn test_effect_size_calculations() {
    ...
    assert!(cohens_d > 1.5 && cohens_d < 2.5,
            "Cohen's d should indicate large effect: {}", cohens_d);
    
    // Test with no difference
    let group3 = group1.clone();
    let cohens_d_zero = calculate_cohens_d(&group1, &group3);
    
    assert!(cohens_d_zero.abs() < 0.1,
            "Cohen's d should be near zero for identical groups: {}", cohens_d_zero);
}
...
fn bootstrap_confidence_interval(data: &[f64], n_bootstrap: usize, confidence: f64) -> (f64, f64) {
    ...
    (bootstrap_means[lower_idx], bootstrap_means[upper_idx.min(n_bootstrap - 1)])
}
...
fn one_sample_t_test(sample: &[f64], null_mean: f64) -> (f64, f64) {
    ...
    let p_value = if n > 30.0 {
        ...
        if t_stat.abs() > 2.0 { 0.05 } else { 0.2 }
    };
...
fn calculate_cohens_d(group1: &[f64], group2: &[f64]) -> f64 {
    ...
    let var1 = group1.iter()
        ...
        .sum::<f64>() / (group1.len() - 1) as f64;
    
    let var2 = group2.iter()
        .map(|x| (x - mean2).powi(2))
        .sum::<f64>() / (group2.len() - 1) as f64;
