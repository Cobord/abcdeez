...
fn test_bayesian_model_initialization() {
    ...
    assert_eq!(model.node_positions.len(), 26, "Should have 26 node positions");
    assert_eq!(model.operation_proficiencies.len(), 6, "Should have 6 operation types");
    ...
    for (key, posterior) in &model.node_positions {
        ...
        let node_index = key.strip_prefix("node_")
            .and_then(|s| s.parse::<usize>().ok())
            .unwrap_or(0);
        
        // Initial mean should match node position from topology
        let expected_position = node_index as f64;
        assert!((posterior.mean - expected_position).abs() < 2.0,
                "Node {} initial mean {} should be near position {}",
                key, posterior.mean, expected_position);
        
        // Initial variance should be 1.0 (moderate uncertainty)
        assert!((posterior.variance - 1.0).abs() < 0.1,
                "Initial variance {} should be near 1.0", posterior.variance);
        
        // Confidence should be 1/(1+variance) ≈ 0.5 for variance=1
        assert!((posterior.confidence - 0.5).abs() < 0.1,
                "Initial confidence {} should be near 0.5", posterior.confidence);
    }
    
    // Verify operation proficiencies start neutral
    for (_op, prof) in &model.operation_proficiencies {
        assert!(prof.mean.abs() < 0.1, "Initial proficiency mean should be near 0");
        assert!((prof.variance - 1.0).abs() < 0.1, "Initial proficiency variance should be near 1");
    }
    
    // Verify chunk boundaries for linear topology
    if matches!(topo.topology_type, crate::topology::TopologyType::Linear) {
        assert_eq!(model.chunk_boundaries.len(), 3, "Should have 3 chunk boundaries");
        let expected_positions = vec![6, 13, 19];
        for (i, boundary) in model.chunk_boundaries.iter().enumerate() {
            assert_eq!(boundary.position, expected_positions[i], 
                      "Chunk boundary {} at wrong position", i);
        }
    }
}
...
fn test_monte_carlo_eig_positive() {
    ...
    assert!(eig_100 >= 0.0, "EIG with 100 samples was negative: {}", eig_100);
    assert!(eig_1000 >= 0.0, "EIG with 1000 samples was negative: {}", eig_1000);
    
    // EIG should be bounded by total entropy
    let total_entropy = model.total_entropy();
    assert!(eig_100 <= total_entropy, "EIG {} exceeds total entropy {}", eig_100, total_entropy);
    assert!(eig_1000 <= total_entropy, "EIG {} exceeds total entropy {}", eig_1000, total_entropy);
    
    // More samples should give more stable estimate (not necessarily larger)
    // But they should be in the same ballpark
    assert!((eig_100 - eig_1000).abs() < eig_100 * 0.5,
            "EIG estimates should be similar: 100 samples={}, 1000 samples={}",
            eig_100, eig_1000);
    
    // EIG should be positive for an informative task
    assert!(eig_1000 > 0.001, 
            "EIG should be positive for informative task, got {}", eig_1000);
}
...
fn test_eig_high_for_uncertain_tasks() {
    ...
    assert!((initial_certain_eig - initial_uncertain_eig).abs() < initial_certain_eig * 0.5,
            "Initially, both tasks should have similar EIG: A={}, X={}",
            initial_certain_eig, initial_uncertain_eig);
    ...
    assert!(certain_eig_after < initial_certain_eig * 0.5,
            "EIG for learned task should decrease: {} -> {}",
            initial_certain_eig, certain_eig_after);
    
    // Uncertain task should have significantly higher EIG than certain task
    assert!(uncertain_eig_after > certain_eig_after * 2.0, 
            "Uncertain task EIG {} should be much higher (>2x) than certain task EIG {}", 
            uncertain_eig_after, certain_eig_after);
}
...
fn test_entropy_calculation() {
    ...
    assert!(initial_entropy > 0.0, "Initial entropy should be positive: {}", initial_entropy);
    ...
    assert!(initial_entropy > expected_entropy * 0.5 && initial_entropy < expected_entropy * 2.0,
            "Initial entropy {} should be near expected {}",
            initial_entropy, expected_entropy);
