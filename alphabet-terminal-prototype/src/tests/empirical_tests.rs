...
fn test_power_law_of_practice() {
    ...
    for trial in 0..50 {
        ...
        let proficiency = learner.operation_proficiencies
            .get("Successor")
    ...
    assert!(b > 0.1 && b < 1.0,
            "Power law exponent should be in typical range [0.1, 1.0]: {}", b);
    
    // Check goodness of fit
    let r_squared = calculate_power_law_r_squared(&response_times, a, b);
    assert!(r_squared > 0.6,
            "Power law should fit reasonably well: R²={:.3}", r_squared);
...
fn test_spacing_effect() {
    ...
    let spaced_retention: f64 = items.iter()
        .filter_map(|item| topo.get_node_by_label(item).map(|n| n.position as usize))
        .map(|idx| {
            spaced_learner.memory_strengths
                .get(&format!("node_{}", idx))
                .map(|m| m.strength)
                .unwrap_or(0.5)
        })
    ...
    assert!(spaced_retention > massed_retention * 1.1,
            "Spaced practice ({:.3}) should be more effective than massed ({:.3})",
            spaced_retention, massed_retention);
}
...
fn test_fan_effect() {
    ...
    for item in &medium_fan_items {
        for _ in 0..2 { // More practice for medium fan
            learner.update_memory_strength(item, true);
        }
    }
    
    for item in &high_fan_items {
        for _ in 0..3 { // Even more practice for high fan
            learner.update_memory_strength(item, true);
        }
    }
    ...
    // Verify fan effect
    assert!(medium_fan_rt > low_fan_rt * 1.1,
            "Medium fan RT ({}) should be slower than low fan ({})",
            medium_fan_rt, low_fan_rt);
    
    assert!(high_fan_rt > medium_fan_rt * 1.1,
            "High fan RT ({}) should be slower than medium fan ({})",
            high_fan_rt, medium_fan_rt);
    
    // RT should increase monotonically with fan
    assert!(low_fan_rt < medium_fan_rt && medium_fan_rt < high_fan_rt,
            "RT should increase with fan: {} < {} < {}",
            low_fan_rt, medium_fan_rt, high_fan_rt);
}
...
fn test_strategy_shift_with_practice() {
    ...
    let early_r = calculate_correlation(
        &early_correlations.iter().map(|(d, _)| *d).collect::<Vec<_>>(),
        &early_correlations.iter().map(|(_, rt)| *rt).collect::<Vec<_>>()
    );
    ...
    let late_r = calculate_correlation(
        &late_correlations.iter().map(|(d, _)| *d).collect::<Vec<_>>(),
        &late_correlations.iter().map(|(_, rt)| *rt).collect::<Vec<_>>()
    );
    
    // Verify strategy shift
    assert!(early_r > 0.7,
            "Early phase should show serial scanning (r={:.3})", early_r);
    
    assert!(late_r < 0.5,
            "Late phase should show direct access (r={:.3})", late_r);
    
    assert!(early_r > late_r + 0.2,
...
fn fit_power_law(data: &[f64]) -> (f64, f64) {
    ...
    let log_n: Vec<f64> = (1..=data.len())
        .map(|i| (i as f64).ln())
        .collect();
    let log_rt: Vec<f64> = data.iter()
        .map(|rt| rt.ln())
        .collect();
    ...
    let numerator: f64 = log_n.iter().zip(log_rt.iter())
        .map(|(ln, lrt)| (ln - mean_log_n) * (lrt - mean_log_rt))
        .sum();
    
    let denominator: f64 = log_n.iter()
        .map(|ln| (ln - mean_log_n).powi(2))
        .sum();
...
fn test_transfer_learning_effect() {
    ...
    let alphabet_prof = learner.operation_proficiencies
        .get("Successor")
        .map(|p| 1.0 / (1.0 + (-p.theta).exp()))
        .unwrap_or(0.5);
    ...
    if let Some(prof) = transfer_learner.operation_proficiencies.get_mut("Successor") {
        prof.theta = learner.operation_proficiencies
            .get("Successor")
    ...
    let transfer_initial = transfer_learner.operation_proficiencies
        .get("Successor")
        .map(|p| 1.0 / (1.0 + (-p.theta).exp()))
        .unwrap_or(0.5);
    
    let naive_initial = naive_learner.operation_proficiencies
        .get("Successor")
        .map(|p| 1.0 / (1.0 + (-p.theta).exp()))
        .unwrap_or(0.5);
    
    // Transfer learner should start with advantage
    assert!(transfer_initial > naive_initial * 1.2,
            "Transfer learner ({:.3}) should have advantage over naive ({:.3})",
            transfer_initial, naive_initial);
    
    // Both should be worse than original domain
    assert!(transfer_initial < alphabet_prof,
