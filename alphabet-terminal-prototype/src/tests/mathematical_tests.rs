...
fn test_ex_gaussian_pdf_formula_correctness() {
    ...
    assert!(pdf_at_mu > 0.1 && pdf_at_mu < 0.3,
            "PDF at mu={} should be reasonable, got {}", params.mu, pdf_at_mu);
    ...
    assert!(pdf_at_zero > 0.2 && pdf_at_zero < 0.3,
    ...
    assert!(pdf_negative < pdf_at_zero,
    ...
    assert!(pdf_tail > 0.0 && pdf_tail < 0.1,
            "PDF should have exponential tail: pdf(5)={}", pdf_tail);
}
...
fn test_kl_divergence_calculation() {
    ...
    assert!(kl_self.abs() < 1e-10,
            "KL divergence with itself should be 0, got {}", kl_self);
    ...
    assert!(kl > 0.0, "KL divergence between different distributions should be positive, got {}", kl);
    ...
    assert!((kl_known - 0.5).abs() < 0.01,
            "KL(N(0,1) || N(1,1)) should be 0.5, got {}", kl_known);
    ...
    assert!((calculated_kl - expected_kl).abs() < 0.001,
            "KL divergence calculation incorrect: expected {}, got {}",
            expected_kl, calculated_kl);
}
...
fn test_adaptive_monte_carlo_convergence() {
    ...
    let task = crate::tasks::Task {
        task_type: crate::tasks::TaskType::Successor { item: "M".to_string() },
        prompt: "".to_string(),
    ...
    assert!(avg_samples < 5000,
            "Should converge before max samples. Average samples used: {}", avg_samples);
    
    // Should use more than minimum samples for non-trivial task
    assert!(avg_samples > 100,
            "Should use more than minimum samples for convergence. Average: {}", avg_samples);
}
...
fn test_numerical_stability_thresholds() {
    ...
    assert!(entropy.is_finite() || entropy == 0.0,
            "Entropy should handle tiny variance gracefully, got {}", entropy);
    ...
    assert_eq!(zero_entropy, 0.0,
              "Zero variance should give zero entropy, got {}", zero_entropy);
    ...
    assert!(pdf_test.is_finite() || pdf_test == 0.0,
            "PDF should handle extreme parameters, got {}", pdf_test);
}
...
fn test_fisher_exact_test_implementation() {
    ...
    assert!(result.p_value < 0.05,
            "Should detect significant difference, p={}", result.p_value);
    
    // Test name should indicate Fisher's test was used
    assert!(result.test_name.contains("Fisher") || result.test_name.contains("Chi"),
            "Test name should indicate test type: {}", result.test_name);
}
...
fn test_memory_decay_formula() {
    ...
    let initial = learner.memory_strengths.get("node_0")
        .map(|m| m.strength)
        .unwrap_or(0.5);
    ...
    assert!(retention < initial,
            "Memory should decay over time: {} -> {}", initial, retention);
    
    // Should follow exponential decay: strength * exp(-decay_rate * time)
    // For 1 hour with moderate decay rate, expect ~70-95% retention
    assert!(retention > initial * 0.6 && retention < initial * 0.98,
