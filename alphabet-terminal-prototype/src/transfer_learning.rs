...
impl TransferLearningSystem {
    ...
    fn compute_isomorphic_mapping(source: &Topology, target: &Topology) -> IsomorphicMapping {
        ...
        if structure_preserved {
            ...
            for i in 0..source.nodes.len().min(target.nodes.len()) {
                node_mapping.insert(
                    source.nodes[i].label.clone(),
                    target.nodes[i].label.clone()
                );
            }
        } else {
    ...
    pub fn transfer_knowledge(&self) -> LearnerModel {
        let mut target_model = LearnerModel::new("transfer_target".to_string(), &self.target_topology);
        
        if let Some(source_model) = &self.source_model {
            match self.transfer_type {
                TransferType::Direct => self.direct_transfer(source_model, &mut target_model),
                TransferType::Structural => self.structural_transfer(source_model, &mut target_model),
                TransferType::Functional => self.functional_transfer(source_model, &mut target_model),
                TransferType::Analogical => self.analogical_transfer(source_model, &mut target_model),
                TransferType::Hierarchical => self.hierarchical_transfer(source_model, &mut target_model),
            }
        }
    ...
    fn direct_transfer(&self, source: &LearnerModel, target: &mut LearnerModel) {
        ...
        for (source_label, target_label) in &self.mapping.node_mapping {
            if let (Some(source_node), Some(target_node)) = (
                self.source_topology.nodes.iter().find(|n| &n.label == source_label),
                self.target_topology.nodes.iter().find(|n| &n.label == target_label)
            ) {
                ...
                if let Some(source_embedding) = source.node_embeddings.get(&source_node.id) {
                    if let Some(target_embedding) = target.node_embeddings.get_mut(&target_node.id) {
                        ...
                        target_embedding.uncertainty = source_embedding.uncertainty / self.mapping.transfer_strength;
                    }
                }
            }
        }
        ...
        for (op_key, source_prof) in &source.operation_proficiencies {
            if let Some(target_prof) = target.operation_proficiencies.get_mut(op_key) {
                ...
                target_prof.practice_count = (source_prof.practice_count as f64 * self.mapping.transfer_strength) as usize;
            }
        }
    }
    ...
    fn structural_transfer(&self, source: &LearnerModel, target: &mut LearnerModel) {
        ...
        if self.mapping.structure_preserved {
            ...
            target.chunk_boundaries = source.chunk_boundaries.iter()
                .map(|cb| crate::learner::ChunkBoundary {
    ...
    fn functional_transfer(&self, source: &LearnerModel, target: &mut LearnerModel) {
        ...
        for (op_key, source_prof) in &source.operation_proficiencies {
            if let Some(target_prof) = target.operation_proficiencies.get_mut(op_key) {
                ...
                target_prof.theta = source_prof.theta * transfer_weight * self.mapping.transfer_strength;
            }
        }
    }
    ...
    fn hierarchical_transfer(&self, source: &LearnerModel, target: &mut LearnerModel) {
        ...
        let avg_proficiency: f64 = source.operation_proficiencies.values()
            .map(|p| p.theta)
            .sum::<f64>() / source.operation_proficiencies.len() as f64;
        ...
        if source.chunk_boundaries.len() > 0 {
            let chunk_ratio = target.node_embeddings.len() as f64 / source.node_embeddings.len() as f64;
            target.chunk_boundaries = source.chunk_boundaries.iter()
                .map(|cb| crate::learner::ChunkBoundary {
    ...
    fn extract_patterns(&self, model: &LearnerModel) -> HashMap<String, f64> {
        ...
        if !model.chunk_boundaries.is_empty() {
            let avg_strength = model.chunk_boundaries.iter()
                .map(|cb| cb.strength)
                .sum::<f64>() / model.chunk_boundaries.len() as f64;
    ...
    fn map_patterns_to_target(&self, source_patterns: &HashMap<String, f64>) -> HashMap<String, f64> {
    ...
    pub fn generate_transfer_task(&mut self) -> Task {
        ...
        let source_item = &self.source_topology.nodes[rand::random::<usize>() % self.source_topology.nodes.len()];
        let target_item = self.mapping.node_mapping.get(&source_item.label)
        ...
        let correct_answer = if let Some(target_node) = self.target_topology.get_node_by_label(target_item) {
    ...
    pub fn measure_transfer_effectiveness(&self, 
