...
use graph_learning_core::{
    ...
    InterventionSystem, InterventionAction, ResponseTimeDistribution
};
...
fn main() {
    ...
    for i in 0..5 {
        ...
        println!("   Task {}: {:?} (difficulty: {:.2})", i + 1, task.task_type, task.difficulty);
        tasks.push(task);
    }
    ...
    println!("   ✅ Adaptive scheduler selected task: {:?}", adaptive_task.task_type);
    ...
    println!("   📊 Ex-Gaussian fit - μ: {:.2}, σ: {:.2}, τ: {:.2}", 
