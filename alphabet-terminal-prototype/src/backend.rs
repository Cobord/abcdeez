...
impl BackendConfig {
    ...
    pub fn from_file(path: &str) -> Result<Self> {
        let content = std::fs::read_to_string(path)
            .context("Failed to read backend config file")?;
...
impl BackendClient {
    ...
    pub fn sync_responses(&mut self) -> Result<()> {
        ...
        anyhow::bail!("Failed to sync after {} attempts: {}", 
    ...
    pub fn get_experiment_config(&self) -> Result<ExperimentConfig> {
        let url = format!("{}/experiments/{}", self.config.api_url, self.config.experiment_id);
