...
impl StrategyMixtureModel {
    ...
    pub fn update_weights(&mut self, task: &Task, success: bool, response_time: f64) {
        ...
        let total_evidence: f64 = self.mixture_weights.iter()
            .zip(&likelihoods)
            .map(|(w, l)| w * l)
            .sum();
    ...
    fn calculate_strategy_likelihood(&self, 
        ...
        let expected_success = match strategy.strategy_type {
            ...
            MixtureStrategyType::Anchoring => {
                if strategy.parameters.anchor_points.contains(&task.correct_answer) {
                    0.95
                } else {
                    0.6
                }
    ...
    pub fn generate_response(&self, task: &Task, model: &LearnerModel) -> (String, f64) {
        ...
        match strategy.strategy_type {
            MixtureStrategyType::Sequential => {
                ...
                let confidence = model.operation_proficiencies
                    .get("Successor")
                    .map(|p| p.theta)
                    .unwrap_or(0.5);
            ...
            MixtureStrategyType::Anchoring => {
                ...
                let is_anchor = strategy.parameters.anchor_points
                ...
                if rng.gen::<f64>() < confidence {
                    ...
                    if !strategy.parameters.anchor_points.is_empty() && rng.gen_bool(0.5) {
                        ...
                        (strategy.parameters.anchor_points[idx].clone(), confidence * 0.3)
                    } else {
                        let idx = rng.gen_range(0..task.options.len());
                        (task.options[idx].clone(), confidence * 0.5)
                    }
            ...
            MixtureStrategyType::Pattern => {
                ...
                let confidence = model.operation_proficiencies
                    .values()
                    .map(|p| p.theta)
                    .sum::<f64>() / model.operation_proficiencies.len() as f64;
    ...
    pub fn estimate_parameters(&mut self, performance_data: &[(Task, bool, f64)]) {
        ...
        for _ in 0..MAX_ITERATIONS {
            ...
            let mut responsibilities = vec![vec![0.0; self.strategies.len()]; performance_data.len()];
            
            for (i, (task, success, rt)) in performance_data.iter().enumerate() {
                let mut likelihoods = Vec::new();
                for strategy in &self.strategies {
                    likelihoods.push(self.calculate_strategy_likelihood(
                        strategy, task, *success, *rt
                    ));
                }
                
                let total: f64 = self.mixture_weights.iter()
                    .zip(&likelihoods)
                    .map(|(w, l)| w * l)
                    .sum();
            ...
            for j in 0..self.strategies.len() {
                self.mixture_weights[j] = responsibilities.iter()
                    .map(|r| r[j])
                    .sum::<f64>() / performance_data.len() as f64;
            }
            
            // Check convergence
            let diff: f64 = old_weights.iter()
    ...
    pub fn get_dominant_strategy(&self) -> &CognitiveStrategy {
        let max_idx = self.mixture_weights
            .iter()
            .enumerate()
    ...
    fn predict_with_strategy(&self, strategy: &CognitiveStrategy, task: &Task) -> (f64, f64) {
        match strategy.strategy_type {
            ...
            MixtureStrategyType::Anchoring => {
                let success = if strategy.parameters.anchor_points.contains(&task.correct_answer) {
                    0.95
                } else {
                    0.6
                };
...
impl StrategyHMM {
    ...
    pub fn viterbi(&self, observations: &[String]) -> Vec<usize> {
        ...
        // First observation
        for i in 0..n_states {
            let emission = self.emission_probs
                .get(&observations[0])
    ...
    pub fn baum_welch(&mut self, observations: Vec<Vec<String>>) {
        ...
        for _ in 0..MAX_ITERATIONS {
            ...
            // Check convergence
            let diff: f64 = old_transitions.iter()
    ...
    fn forward_backward(&self, observations: &[String]) 
