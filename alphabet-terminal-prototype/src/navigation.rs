...
impl ConstrainedNavigator {
    pub fn new(topology: Topology) -> Self {
        ...
        if topology.nodes.len() == 26 {
            for node in &topology.nodes {
                ...
                props.insert("quadrant".to_string(), (node.position as usize / 7).to_string());
    ...
    pub fn find_constrained_path(
        &self,
        task: &ConditionalNavigationTask
    ) -> Option<Vec<String>> {
        ...
        f_score.insert(start_node.id.clone(), self.heuristic(&start_node.id, &goal_node.id));
    ...
    fn node_matches_filter(&self, node_label: &str, filter: &PropertyFilter) -> bool {
        match filter {
            PropertyFilter::NodeType(node_type) => {
            ...
            PropertyFilter::Custom(key, value) => {
                self.node_properties
                    .get(node_label)
    ...
    fn heuristic(&self, from: &str, to: &str) -> f64 {
        ...
        self.topology.get_distance(from, to)
            .map(|d| d as f64)
    ...
    pub fn generate_conditional_task(&self, task: ConditionalNavigationTask) -> Task {
        ...
        let constraints_str = task.constraints.iter()
            .map(|c| match c {
                NavigationConstraint::AvoidNodes(nodes) => 
                    format!("avoiding {}", nodes.join(", ")),
                NavigationConstraint::RequireNodes(nodes) => 
                    format!("passing through {}", nodes.join(", ")),
                NavigationConstraint::MaxDistance(d) => 
                    format!("in at most {} steps", d),
                NavigationConstraint::MinDistance(d) => 
                    format!("in at least {} steps", d),
...
impl MultiHopNavigator {
    ...
    pub fn find_nodes_at_distances(
        &self,
        constraints: Vec<(String, usize)>
    ) -> Vec<String> {
    ...
    pub fn generate_distance_constraint_task(&self) -> Task {
        ...
        let prompt = format!(
            "What nodes are exactly {} steps from '{}'?",
            dist1, anchor
        );
        ...
        Task {
            task_type: TaskType::Index { item: anchor.clone() },
            prompt,
            correct_answer,
            options: vec![],
            difficulty: 0.6,
