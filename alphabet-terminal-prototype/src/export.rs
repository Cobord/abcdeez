...
impl LearnerDataExport {
    ...
    pub fn export_for_r(&self, base_path: &Path, include_script: bool) -> std::io::Result<()> {
        ...
        if include_script {
            ...
            let codebook_path = base_path.with_file_name(
                format!("{}_codebook.txt", base_path.file_stem().unwrap().to_string_lossy())
            );
...
"#, 
            ...
            self.sessions.iter().map(|s| s.summary.mean_rt_ms).sum::<f64>() / self.sessions.len() as f64,
            self.sessions.len(),
    ...
    fn format_error_summary(&self) -> String {
        ...
        summary.push_str(&format!("Error rate: {:.1}%\n\n", self.error_patterns.error_rate * 100.0));
        
        summary.push_str("Most common confusions:\n");
        for (i, (expected, actual, count)) in self.error_patterns.common_confusions.iter().enumerate().take(5) {
            summary.push_str(&format!("{}. Expected '{}' → Got '{}' ({} times)\n", i + 1, expected, actual, count));
        }
...
impl PopulationAnalyzer {
    ...
    pub fn export_for_python_analysis(&self, base_path: &Path, notebook: bool) -> std::io::Result<()> {
        std::fs::create_dir_all(base_path)?;
...
"#, 
            chrono::Utc::now().format("%Y-%m-%d %H:%M:%S"),
            self.learners.len(),
            self.learners.iter().map(|l| l.sessions.len()).sum::<usize>(),
