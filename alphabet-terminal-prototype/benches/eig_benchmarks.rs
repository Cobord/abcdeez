...
fn benchmark_eig_different_sample_sizes(c: &mut Criterion) {
    ...
    for sample_size in [100, 500, 1000, 2000, 5000].iter() {
        group.bench_with_input(BenchmarkId::from_parameter(sample_size), sample_size, 
            |b, &sample_size| {
                b.iter(|| {
                    model.monte_carlo_eig(black_box(&task), black_box(sample_size))
                });
            });
    }
    group.finish();
...
fn benchmark_bayesian_update(c: &mut Criterion) {
    ...
    c.bench_function("Bayesian update", |b| {
        b.iter(|| {
            model.update_with_response(black_box(response.clone()))
        });
    });
}
...
fn benchmark_adaptive_monte_carlo(c: &mut Criterion) {
    ...
    let task = Task {
        ...
        options: vec!["F".to_string(), "G".to_string(), "H".to_string(), "I".to_string()],
        difficulty: 0.7,
    ...
    c.bench_function("Adaptive Monte Carlo EIG", |b| {
        b.iter(|| {
            model.adaptive_monte_carlo_eig(black_box(&task))
        });
    });
}
...
fn benchmark_topology_operations(c: &mut Criterion) {
    ...
    c.bench_function("Distance calculation", |b| {
        b.iter(|| {
            topo.get_distance(black_box("A"), black_box("Z"))
        });
    });
    
    c.bench_function("Successor lookup", |b| {
        b.iter(|| {
            topo.get_successor(black_box("M"))
        });
    });
    
    c.bench_function("K-jump calculation", |b| {
        b.iter(|| {
            topo.get_k_jump(black_box("A"), black_box(5))
        });
    });
}
...
fn benchmark_large_topology_performance(c: &mut Criterion) {
    ...
    let large_items: Vec<String> = (0..1000)
    ...
    c.bench_function("EIG on large topology", |b| {
        b.iter(|| {
            model.monte_carlo_eig(black_box(&task), black_box(100))
        });
    });
    
    c.bench_function("Distance on large topology", |b| {
        b.iter(|| {
            large_topo.get_distance(black_box("item_0100"), black_box("item_0800"))
        });
    });
}
...
fn benchmark_performance_requirements(c: &mut Criterion) {
    let model = BayesianLearnerModel::new(&Topology::alphabet());
    let task = Task {
        task_type: TaskType::Index { item: "M".to_string() },
