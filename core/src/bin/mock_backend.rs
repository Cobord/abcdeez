...
async fn register_participant(
    ...
    println!(
        "✓ Registered participant: {} in group {}",
...
async fn receive_metrics(Json(payload): Json<serde_json::Value>) -> impl IntoResponse {
...
async fn log_error(Json(payload): Json<serde_json::Value>) -> impl IntoResponse {
...
async fn next_task(Json(_payload): Json<serde_json::Value>) -> impl IntoResponse {
    // Return no content for adaptive mode (let client generate tasks)
    StatusCode::NO_CONTENT
}

async fn get_experiment(Path(id): Path<String>) -> impl IntoResponse {
    Json(json!({
        "experiment_id": id,
...
async fn get_stats(State(state): State<AppState>) -> impl IntoResponse {
    ...
    let total_correct: usize = responses.iter().filter(|r| r.correct).count();

    let avg_rt: f64 = if !responses.is_empty() {
        responses.iter().map(|r| r.rt_ms as f64).sum::<f64>() / responses.len() as f64
    } else {
        0.0
    };
