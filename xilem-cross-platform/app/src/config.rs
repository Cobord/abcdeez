    ...
    fn get_config_dir() -> Result<std::path::PathBuf> {
        ...
        #[cfg(target_os = "linux")]
        {
            if let Ok(xdg_config) = std::env::var("XDG_CONFIG_HOME") {
                ...
                let home = std::env::var("HOME")
                    .map_err(|_| anyhow::anyhow!("Could not find HOME directory"))?;
                Ok(std::path::PathBuf::from(home).join(".config").join("abcdeez"))
            }
        }
...
mod tests {
    ...
    fn test_default_config() {
        ...
        assert_eq!(config.api.base_url, "https://abcdeez.fg-goose.online/api/v1");
    ...
    fn test_config_file_operations() {
        ...
        assert_eq!(manager.config().api.base_url, "https://custom.example.com/api/v1");
    }
    ...
    fn test_env_var_override() {
        ...
        assert_eq!(manager.config().api.base_url, "https://env.example.com/api/v1");
