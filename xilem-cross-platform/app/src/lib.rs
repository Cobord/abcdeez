...
use xilem::{
    view::{button, button_any_pointer, checkbox, flex, label, prose, textbox, PointerButton, Axis},
    AnyWidgetView, Color, EventLoopBuilder, TextAlignment, WidgetView, Xilem,
};
...
fn app_logic(data: &mut AppData) -> impl WidgetView<AppData> {
    ...
    flex((
        flex((
            label("Label")
                .brush(masonry::palette::css::REBECCA_PURPLE)
