use crate::topology::{Topology, TopologyType, Node, Edge};
...
impl MusicTheory {
    ...
    fn pitch_class_to_note(pc: u8, use_sharps: bool) -> String {
        let sharps = vec!["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"];
        let flats = vec!["C", "Db", "D", "Eb", "E", "F", "Gb", "G", "Ab", "A", "Bb", "B"];
...
impl MusicTaskGenerator {
    ...
    pub fn generate_interval_task(&self, from: String, to: String) -> Task {
        ...
        let interval = self.theory.intervals
        ...
        let mut unique_options: Vec<String> = options.into_iter()
    ...
    pub fn generate_scale_degree_task(&self, scale_degree: usize) -> Task {
        let note = self.theory.topology.nodes
            .get(scale_degree - 1)
            .map(|n| n.label.clone())
        ...
        let options: Vec<String> = self.theory.topology.nodes
            .iter()
            .map(|n| n.label.clone())
            .collect();
    ...
    pub fn generate_chord_progression_task(&self, position: usize) -> Task {
        let chord = self.theory.topology.nodes
            .get(position)
            .map(|n| n.label.clone())
            .unwrap_or("Unknown".to_string());
        
        let next_chord = self.theory.topology.nodes
            .get(position + 1)
            .map(|n| n.label.clone())
    ...
    pub fn generate_transposition_task(&self, note: String, interval: i32) -> Task {
        ...
        Task {
            task_type: TaskType::KJump { start: note, k: interval },
            prompt,
