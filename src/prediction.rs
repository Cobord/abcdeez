...
impl PerformancePredictor {
    ...
    fn fit_power_law(&mut self, data: &[(usize, f64)]) {
        ...
        // Estimate asymptote
        let last_values: Vec<f64> = data.iter()
            .rev()
            .take(5)
            .map(|(_, p)| *p)
            .collect();
    ...
    fn fit_biexponential(&mut self, data: &[(usize, f64)]) {
        ...
        if !early_data.is_empty() {
            let early_rate = (early_data.last().unwrap().1 - early_data[0].1) 
                / early_data.len() as f64;
        ...
        if !late_data.is_empty() {
            let late_rate = (late_data.last().unwrap().1 - late_data[0].1)
                / late_data.len() as f64;
    ...
    pub fn predict_range(&self, start: usize, end: usize) -> Vec<(usize, f64)> {
        (start..=end)
            .map(|t| (t, self.predict(t)))
            .collect()
    }
    
    pub fn confidence_interval(&self, trial: usize, alpha: f64) -> (f64, f64) {
...
impl ScheduleOptimizer {
    ...
    fn estimate_current_mastery(&self) -> f64 {
        ...
        let proficiencies: Vec<f64> = self.learner_model
...
impl TrajectoryAnalyzer {
    ...
    pub fn analyze_learning_trajectory(&self) -> LearningTrajectory {
        ...
        LearningTrajectory {
            ...
            improvement_rate,
            final_performance: self.responses.last()
