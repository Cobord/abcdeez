...
// Always prefer native if both are set, otherwise use web if only web is set, otherwise define our own.
...
// If neither native nor web, define a fallback Color type.
#[cfg(all(not(feature = "xilem-native"), not(feature = "xilem-web")))]
#[derive(Debug, Clone, Copy)]
pub struct Color {
    pub r: u8,
    pub g: u8,
